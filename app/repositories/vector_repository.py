...
class VectorRepository:
    ...
    @staticmethod
    def similarity_search(query_embedding, course_id, threshold=0.4, limit=5, module_id=None):
        """
        ...
        try:
            if module_id:
                # Search within specific module by ref_module_id
                cursor.execute(
                    """
                    ...
                    LIMIT %s
                    """,
                    (query_embedding, int(course_id), int(module_id), query_embedding, float(threshold), int(limit))
                )
            else:
                # Search across all modules in course
                cursor.execute(
                    """
                    SELECT c.chunk_text, (1 - (v.embedding <=> %s::vector)) as similarity 
                    FROM chunks c
                    ...
                    LIMIT %s
                    """,
                    (query_embedding, int(course_id), query_embedding, float(threshold), int(limit))
                )
